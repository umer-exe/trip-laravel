@php
    $isModel = $tour instanceof \App\Models\Tour;
    $title = $isModel ? $tour->title : data_get($tour, 'title');
    $slug = $isModel ? $tour->slug : data_get($tour, 'slug');
    $location = $isModel ? $tour->location : data_get($tour, 'location');
    $duration = $isModel ? $tour->duration : data_get($tour, 'duration');
    $price = (float) ($isModel ? $tour->price : data_get($tour, 'price', 0));
    $type = $isModel ? $tour->type : data_get($tour, 'type', 'domestic');
    $thumbnail = $isModel
        ? ($tour->thumbnail_image ?? $tour->banner_image)
        : (data_get($tour, 'thumbnail_image') ?? data_get($tour, 'image'));
    $imageUrl = $thumbnail ? asset($thumbnail) : 'https://placehold.co/600x400?text=Atlas+Tours';
@endphp
...
<div class="bg-white rounded-lg shadow-md overflow-hidden hover:shadow-xl transition transform hover:-translate-y-1 flex flex-col h-full">
    ...
    <div class="p-5 flex flex-col flex-grow">
        ...
        <div class="space-y-2">
            <a href="{{ route('tours.show', $slug) }}" class="block w-full bg-indigo-600 text-white text-center py-2 rounded-lg hover:bg-indigo-700 transition font-medium">
                View Details
            </a>
        </div>
    </div>
